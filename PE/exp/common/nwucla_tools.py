...
def eval_singleclip_gt_bbox_generator(model, datagen, verbose=1):
    ...
    if verbose:
        printc(WARNING, 'nwucla, single-clip, GT bbox, action acc.%:')
...
def eval_multiclip_dataset(model, nwucla, subsampling, bboxes_file=None,
        logdir=None, verbose=1):
    """If bboxes_file if not given, use ground truth bounding boxes."""

    num_samples = nwucla.get_length(TEST_MODE)
    num_blocks = len(model.outputs)

    """Save and reset some original configs from the dataset."""
    org_hflip = nwucla.dataconf.fixed_hflip
    org_use_gt_bbox = nwucla.use_gt_bbox

    cnt_corr = 0
    cnt_total = 0

    action_shape = (num_samples,) + nwucla.get_shape('nwuclaaction')
    ...
    for i in range(num_samples):
        ...
        frame_list = nwucla.get_clip_index(i, TEST_MODE, subsamples=[subsampling])
        ...
        for f in range(len(frame_list)):
            for hflip in range(2):
                preds_clip = []
                try:
                    nwucla.dataconf.fixed_hflip = hflip # Force horizontal flip
                    ...
                    data = nwucla.get_data(i, TEST_MODE, frame_list=frame_list[f],
                            bbox=bbox)
                    a_true[i, :] = data['nwuclaaction']
    ...
    if verbose:
        printcn(WARNING, 'nwucla, multi-clip. ' + bboxes_info + '\n')
        printcn(WARNING, np.array2string(np.array(scores), precision=2))
        printcn(WARNING, 'nwucla best: %.2f' % max(scores))

    nwucla.dataconf.fixed_hflip = org_hflip
    nwucla.use_gt_bbox = org_use_gt_bbox

    return scores


class nwuclaEvalCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.logdir is not None:
            ...
            with open(os.path.join(self.logdir, 'nwucla_val.json'), 'w') as f:
