...
def nwucla_load_annotations(dataset_path, eval_mode='cs',
    ...
    nwuclad_numpy_dir = os.path.join(dataset_path, 'nwuclargb+d_numpy')
    nwuclad_images_dir = os.path.join(dataset_path, image_prefix)
    for d in [nwuclad_numpy_dir, nwuclad_images_dir]:
        assert os.path.isdir(d), \
            f'Error: check your nwucla dataset! `{d}` not found!'
            ...
            for p in range(1,num_P+1):
                for r in range(1,num_R+1):
                    for a in range(1,num_A+1):
                        ...
                        filename = os.path.join(nwuclad_numpy_dir,
                                sequence_id + '.npy')
...
class nwucla(object):
    def __init__(self, dataset_path, dataconf, poselayout=pa20j3d,
        ...
        try:
            self.sequences, self.frame_idx, self.seq_ids, self.actions = \
                    nwucla_load_annotations(dataset_path, num_S=num_S)
        except:
            warning('Error loading nwucla RGB+D dataset!')
            raise

    def get_data(self, key, mode, frame_list=None, bbox=None):
        """Method to load nwucla samples specified by mode and key,
        ...
        action = np.zeros(self.get_shape('nwuclaaction'))
        action[act - 1] = 1.

        output['seq_idx'] = seq_idx
        output['frame_list'] = frame_list
        output['nwuclaaction'] = action
    ...
    def get_pose_annot(self, frames):
        ...
        p = p[:, self.poselayout.map_from_nwucla, :].copy()
