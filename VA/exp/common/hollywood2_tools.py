...
def eval_singleclip_gt_bbox(model, x_te, action_te, batch_size=1, verbose=1):
    ...
    if verbose:
        printc(WARNING, 'hollywood2, single-clip, action acc.%:')
...
def eval_multiclip_dataset(model, hollywood2, subsampling, bboxes_file=None,
        logdir=None, verbose=1):
    """If bboxes_file if not given, use ground truth bounding boxes."""

    num_samples = hollywood2.get_length(TEST_MODE)
    num_blocks = len(model.outputs)

    """Save and reset some original configs from the dataset."""
    org_hflip = hollywood2.dataconf.fixed_hflip

    cnt_corr = 0
    cnt_total = 0

    action_shape = (num_samples,) + hollywood2.get_shape('hollywood2')
    ...
    for i in range(num_samples):
        ...
        frame_list = hollywood2.get_clip_index(i, TEST_MODE, subsamples=[subsampling])
        ...
        for f in range(len(frame_list)):
            for hflip in range(2):
                preds_clip = []
                try:
                    hollywood2.dataconf.fixed_hflip = hflip # Force horizontal flip

                    """Load clip and predict action."""
                    data = hollywood2.get_data(i, TEST_MODE, frame_list=frame_list[f])
                    a_true[i, :] = data['hollywood2']
    ...
    if verbose:
        printcn(WARNING, 'hollywood2, multi-clip.\n')
        printcn(WARNING, np.array2string(np.array(scores), precision=2))
        printcn(WARNING, 'hollywood2 best: %.2f' % max(scores))

    hollywood2.dataconf.fixed_hflip = org_hflip

    return scores


class hollywood2EvalCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.logdir is not None:
            ...
            with open(os.path.join(self.logdir, 'hollywood2_val.json'), 'w') as f:
