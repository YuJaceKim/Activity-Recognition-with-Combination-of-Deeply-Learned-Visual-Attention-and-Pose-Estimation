...
def eval_singleclip_gt_bbox_generator(model, datagen, verbose=1):
    ...
    if verbose:
        printc(WARNING, 'ucf11, single-clip, GT bbox, action acc.%:')
...
def eval_multiclip_dataset(model, ucf11, subsampling, bboxes_file=None,
        logdir=None, verbose=1):
    """If bboxes_file if not given, use ground truth bounding boxes."""

    num_samples = ucf11.get_length(TEST_MODE)
    num_blocks = len(model.outputs)

    """Save and reset some original configs from the dataset."""
    org_hflip = ucf11.dataconf.fixed_hflip
    org_use_gt_bbox = ucf11.use_gt_bbox

    cnt_corr = 0
    cnt_total = 0

    action_shape = (num_samples,) + ucf11.get_shape('ucf11action')
    ...
    for i in range(num_samples):
        ...
        frame_list = ucf11.get_clip_index(i, TEST_MODE, subsamples=[subsampling])
        ...
        for f in range(len(frame_list)):
            for hflip in range(2):
                preds_clip = []
                try:
                    ucf11.dataconf.fixed_hflip = hflip # Force horizontal flip
                    ...
                    data = ucf11.get_data(i, TEST_MODE, frame_list=frame_list[f],
                            bbox=bbox)
                    a_true[i, :] = data['ucf11action']
    ...
    if verbose:
        printcn(WARNING, 'ucf11, multi-clip. ' + bboxes_info + '\n')
        printcn(WARNING, np.array2string(np.array(scores), precision=2))
        printcn(WARNING, 'ucf11 best: %.2f' % max(scores))

    ucf11.dataconf.fixed_hflip = org_hflip
    ucf11.use_gt_bbox = org_use_gt_bbox

    return scores


class ucf11EvalCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.logdir is not None:
            ...
            with open(os.path.join(self.logdir, 'ucf11_val.json'), 'w') as f:
